...
LUA_SHA1 = get_hash(os.path.join(CWD, 'bufftoon.lua'))
...
class WgetArgs(object):
    def realize(self, item):
        ...
        for item_name in item['item_name'].split('\0'):
            ...
            if item_type == 'series':
                wget_args.extend(['--warc-header', 'bufftoon-series: '+item_value])
                wget_args.append('https://bufftoon.plaync.com/series/'+item_value)
            elif item_type == 'episode':
                series, episode = item_value.split(':')
                wget_args.extend(['--warc-header', 'bufftoon-episode: '+episode])
                wget_args.append('https://bufftoon.plaync.com/series/{}/{}'.format(series, episode))
            elif item_type == 'asset':
                url = 'https://' + item_value
                wget_args.extend(['--warc-header', 'bufftoon-asset: '+url])
...
project = Project(
    title=TRACKER_ID,
    project_html='''
        <img class="project-logo" alt="Project logo" src="https://wiki.archiveteam.org/images/6/65/Bufftoon-icon.png" height="50px" title=""/>
        <h2>버프툰 <span class="links"><a href="https://bufftoon.plaync.com/">Website</a> &middot; <a href="http://tracker.archiveteam.org/bufftoon/">Leaderboard</a> &middot; <a href="https://wiki.archiveteam.org/index.php/버프툰">Wiki</a></span></h2>
        <p>Archiving 버프툰 again.</p>
    '''
)
